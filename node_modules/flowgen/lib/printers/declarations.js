...
const interfaceDeclaration = (nodeName, node, modifier) => {
  ...
  const str = `${modifier}${type} ${nodeName}${printers.common.generics(node.typeParameters)} ${type === "type" ? "= " : ""}${interfaceType(node, false, type === "type")} ${heritage}`;
  return str;
};
...
const classDeclaration = (nodeName, node) => {
  ...
  const str = `declare ${printers.relationships.exporter(node)}class ${nodeName}${printers.common.generics(node.typeParameters)} ${heritage} ${interfaceType(node, true)}`;
  return str;
};
